...
class AuthChoice extends Widget
{
    ...
    public function run() {
        ...
        $form = ActiveForm::begin(['action' =>array_merge($this->getBaseAuthUrl(), ['id' => $model->userId]), 'method' => 'post']);
        ...
        ])->label(false)->dropDownList(\yii\helpers\ArrayHelper::map($model->getAvailable(),'name','name', 'type'), [
            'groups'=>[
                1=>['label'=>'Roles'],
                2=>['label'=>'Permissions'],
            ],
        ...
        // Html::submitInput('Add Child', ['name'=>'addItem', 'class' => 'btn btn-success']);
